...
events_df = pd.DataFrame({
    'ds': pd.to_datetime(['2025-02-01']),
    'event': ['budget_day']
})
holidays = pd.DataFrame({
    'ds': pd.to_datetime(['2025-02-01'] * 30000),  # triple weight
    'holiday': ['budget_day'] * 30000
})


model.add_seasonality(name='quaterly',period = 91.5,fourier_order= 5)
model.add_seasonality(name = 'monthly',period = 30.5, fourier_order = 5)
model.add_seasonality(name = 'weakly',period = 7, fourier_order = 5)
model.fit(df)
